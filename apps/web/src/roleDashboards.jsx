...
// ---------------- Super Admin ----------------
export function SuperAdminHome({ data, loading }) {
  const { t } = useI18n();
  ...
  return (
    <>
      ...
      <Card title={t("sa.planMix")}>
        {loading ? <LoadingSkeleton /> : <DonutChart data={planMix} />}
      </Card>

      <Card title={t("sa.platformAudit")}>
        {loading ? <LoadingSkeleton /> : <AuditFeed entries={data.platformAuditFeed} showTenant />}
      </Card>
        ...
        {loading ? (
          ...
          <DataTable
            ...
            columns={[
              ...
              { key: "score", header: t("co.score"),
                value: (r) => r.compliance_score,
                render: (r) => (
...
// ---------------- Company Admin ----------------
export function CompanyAdminHome({ data, loading, alerts, calendar }) {
  const { t } = useI18n();
  return (
    <>
      <Card title={t("ca.scores")} wide>
        {loading ? <KpiSkeleton count={3} /> : <ScoresRow summary={data.dashboardSummary} />}
      </Card>
      ...
      <Card title={t("notif.title")}>
        {loading ? <LoadingSkeleton /> : <NotificationsCenter alerts={alerts} />}
      </Card>

      <Card title={t("calendar.title")}>
        {loading ? <LoadingSkeleton /> : <MiniCalendar events={calendar} />}
      </Card>

      <Card title={t("audit.feed")}>
        {loading ? <LoadingSkeleton /> : <AuditFeed entries={data.auditFeed} />}
      </Card>
...
// ---------------- Project Manager ----------------
export function ProjectManagerHome({ data, loading, alerts, calendar, mutate, canApprove }) {
  const { t } = useI18n();
  ...
  return (
    <>
      ...
      {!loading && <ProjectMap sites={data.sites} />}
